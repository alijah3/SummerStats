...
plot1.1 <- ggplot(data = data,
  ...
  scale_fill_manual(values=c("darkgreen","lightskyblue"),
                    guide=F)+
...
plot3<-ggplot(data=data,
              aes(x=agree,
                  y=guilt))+
  labs(x="Participant Agreeablbeness",
       y="Guilt")+
  theme_gray(base_size = 20)+
  geom_point()+
...
plot5.1<-ggplot(data=dataMelt,
  ...
  theme_gray(base_size = 20)+
  scale_x_discrete(labels=c("Partner","Self"))+
  scale_fill_manual(values=c("forestgreen","lightskyblue"),
                    labels=c("Before Shock","After Shock"))+
  geom_violin(position = position_dodge(.9))+
  geom_boxplot(position = position_dodge(.9),
               width=.1)
